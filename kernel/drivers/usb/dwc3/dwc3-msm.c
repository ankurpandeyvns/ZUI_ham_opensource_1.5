	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (vout) {
			...
			if (!dcd && prop_chg_detect) {
				...
				printk("%s: This is a floated charger.\n", __func__);
				g_floated_charger = 1;
			} else {
				printk("%s: SDP charger?\n", __func__);
...
void dwc3_qscpatch_set(struct dwc3 *dwc)
{
	struct dwc3_msm *mdwc = dev_get_drvdata(dwc->dev->parent);

	mdwc->hsphy_init_seq = mdwc->id_state?mdwc->hsphy_init_seq_device:mdwc->hsphy_init_seq_host;
	dwc3_msm_write_readback(mdwc->base, PARAMETER_OVERRIDE_X_REG, 0x03FFFFFF,
			mdwc->hsphy_init_seq & 0x03FFFFFF);
	printk("====%s:%d  hsphy_init_seq = 0x%08x\n",__func__,__LINE__,mdwc->hsphy_init_seq);
}
EXPORT_SYMBOL(dwc3_qscpatch_set);
...
#define USB20_EYE_PATTEN
#ifdef USB20_EYE_PATTEN
static ssize_t qscpatch_override_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "device:0x%08x host:0x%08x\n",mdwc->hsphy_init_seq_device,mdwc->hsphy_init_seq_host);
}

static ssize_t qscpatch_override_store(struct device *dev,
		...
		*buf, size_t size)
{
	...
	mdwc->hsphy_init_seq = val;
	mdwc->hsphy_init_seq_device = val;
	mdwc->hsphy_init_seq_host = val;
	//dwc3_msm_write_readback(mdwc->base,
	//		PARAMETER_OVERRIDE_X_REG, 0x03FFFFFF,
	//		val & 0x03FFFFFF);
...
static DEVICE_ATTR(qscpatch_override, S_IRUGO | S_IWUSR, qscpatch_override_show,
		qscpatch_override_store);
#endif
...
static int __devinit dwc3_msm_probe(struct platform_device *pdev)
{
...
#ifdef USB20_EYE_PATTEN
	device_create_file(&pdev->dev, &dev_attr_qscpatch_override);
#endif
	...
	if (of_property_read_u32(node, "qcom,dwc-hsphy-init-device",
						&mdwc->hsphy_init_seq_device))
		dev_dbg(&pdev->dev, "unable to read hsphy init seq\n");
	else if (!mdwc->hsphy_init_seq_device)
		dev_warn(&pdev->dev, "incorrect hsphyinitseq.Using PORvalue\n");

	if (of_property_read_u32(node, "qcom,dwc-hsphy-init-host",
						&mdwc->hsphy_init_seq_host))
		dev_dbg(&pdev->dev, "unable to read hsphy init seq\n");
	else if (!mdwc->hsphy_init_seq_host)
		dev_warn(&pdev->dev, "incorrect hsphyinitseq.Using PORvalue\n");

	mdwc->hsphy_init_seq = mdwc->hsphy_init_seq_device;
