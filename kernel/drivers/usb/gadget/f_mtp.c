...
//extern void mtp_change_mpctl(char *buf);
static bool mtp_change_cpu_freq = false;
...
static void receive_file_work(struct work_struct *data)
{
	...
	//increase 4 CPU core frequence 
	if(cdev->gadget->speed == USB_SPEED_SUPER) {
		printk("------------------------yangshaoying: superspeed----------------\n");
		//mtp_change_mpctl("41");
		mtp_change_cpu_freq = true;
	}else{
		printk("------------------------yangshaoying: %s----------------\n",usb_speed_string(cdev->gadget->speed));
	}
	...
	//resume 4 CPU core frequence 
	if(mtp_change_cpu_freq == true) {
		mtp_change_cpu_freq = false;
		//mtp_change_mpctl("0");
	}
