...
#define DIAG_ENABLE_PASSWORD  "shenqi"
static ssize_t diag_pstring_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct usb_diag_ch *ch;

	if (size != sizeof(DIAG_ENABLE_PASSWORD)){
		diag_ok = false;
	}else{
		if(memcmp(buf,DIAG_ENABLE_PASSWORD,size-1)){
			diag_ok = false;
		}else{
			diag_ok = true;
		}
	...
	list_for_each_entry(ch, &usb_diag_ch_list, list) {
		struct diag_context *dev= ch->priv_usb;
		if (dev && dev->ch && dev->ch->notify){
			if(diag_ok){
...
static DEVICE_ATTR(pstring, S_IWUSR, NULL, diag_pstring_store);
