...
EXPORT_SYMBOL(qpnp_kpdpwr_bark_list);
BLOCKING_NOTIFIER_HEAD(qpnp_kpdpwr_bark_list);
#endif

static void kpdpwr_bark_work_func(struct work_struct *work)
{
#ifdef QPNP_KPDPWR_BARK_NOTIFIER
	blocking_notifier_call_chain(&qpnp_kpdpwr_bark_list, QPNP_PON_MSG_KPDPWR_BARK, NULL);
#endif
	return;
}
...
static irqreturn_t qpnp_kpdpwr_bark_irq(int irq, void *_pon)
{
	...
	printk(KERN_INFO "kpdpwr hard reset bark irq is comming..\n");
	schedule_delayed_work(&pon->kpdpwr_bark_work, msecs_to_jiffies(10));
...
set the emergent mode by press power key+vol down 7 seconds
	...
	S2 CNTL2: enable
*/
int qpnp_pon_set_emergent_restart_mode(int on)
{
	struct qpnp_pon *pon = sys_reset_dev;
	int rc = 0;
	...
	pr_info("%s: set the emergent mode over(%d)\n",__func__, on);
	return rc;
}
