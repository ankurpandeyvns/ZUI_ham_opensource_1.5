...
static int acc_tuning;
extern int get_acc_tuning_value(struct msm_fb_data_type *mfd);
...
static int ct_mode=2;
static int ce_mode=5;
...
static int set_mode(struct msm_fb_data_type *mfd, struct panel_effect_data *panel_data, int index)
{
	...
	if(!first_set){
		first_set = 1;
		acc_tuning = get_acc_tuning_value(mfd);
		printk("%s: acc_tuning value is %d\n", __func__,acc_tuning);
		if((index == 2) && ((acc_tuning == 3) || (lcdmodule == 2)))
			return 0;
	}
	...
	printk("%s: ctmode is %s,cemode is %s\n", __func__,
			panel_data->mode_data->mode[ct_mode].name,panel_data->mode_data->mode[ce_mode].name);
...
static int lcd_set_effect(struct msm_fb_data_type *mfd, struct panel_effect_data *panel_data, int index, int level)
{
	...
	if(index == 0){//set cabc+sre
		if(ce_mode == 5 && level<2)//level0&level1 for ce_bright_mode, level2&level3 for ce default mode
			level += 2;
		...
		void (*mdss_dsi_panel_cmds_send)(struct mdss_dsi_ctrl_pdata *ctrl,struct dsi_panel_cmds *pcmds))
{
	...
	//ct_mode 2 is default mode, use IC default on settings
	if(ct_mode == 1 || ct_mode == 3 || ((ct_mode == 2) && !acc_tuning && (lcdmodule == 1)))
		temp = copy_mode_code(panel_data, temp, ct_mode, &cnt);
	//temp = copy_mode_code(panel_data, temp, ce_mode, &cnt);
	...
	lcd_effect_info("%s cmds_send finished\n", __func__);
