...
extern void anc_headset_report(unsigned char insert);
/*
 *
 */
...
static int cclogic_get_chip(struct cclogic_dev *cclogic_dev)
{
	...
	if(cclogic_dev->ops){
		pr_info("%s select chip:%s,ret=%d\n",__func__,cclogic_dev->ops->chip_name,ret);
	}else{
		pr_err("%s select chip: nothing\n",__func__);
	}
...
work_end:
	...
	if(ret || !gpio_get_value(cclogic_priv->platform_data->irq_gpio)){
		retries++;
...
{
	return cclogic_priv?cclogic_priv->shenqi_audio:false;
}
EXPORT_SYMBOL(cclogic_device_is_shenqiaudio);
bool cclogic_device_is_otg(void)
{
	return cclogic_priv?cclogic_priv->is_otg:false;
}
EXPORT_SYMBOL(cclogic_device_is_otg);
