...
static int bmg160_parse_dt(struct device *dev,
			struct bmg_client_data *data)
{
	struct device_node *np = dev->of_node;
	int ret = 0;

	ret = of_get_named_gpio(np, "bmg160,irq-gpio", 0);
	if (ret < 0) {
		PERR("failed to get \"bmg160,irq-gpio\"");
		goto err;
	}
	data->irq_gpio = ret;
...
static int bmg160_power_on(struct bmg_client_data *data, bool on)
{
	int rc = 0;
...
static int bmg160_power_init(struct bmg_client_data *data, bool on)
{
	int rc;
...
static int bmg_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = bmg160_parse_dt(&client->dev, client_data);
	if (err < 0) {
                PERR("failed to parse device tree: %d", err);
		goto exit_err_clean;
        }
	PINFO("irq_gpio = %d", client_data->irq_gpio);

	err = bmg160_power_init(client_data, true);
	if (err) {
		PERR("power init failed");
		goto exit_err_clean;
	}

	err = bmg160_power_on(client_data, true);
	if (err) {
