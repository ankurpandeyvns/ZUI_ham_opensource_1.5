...
static int bmm050_parse_dt(struct device *dev,
			struct bmm_client_data *data)
{
	struct device_node *np = dev->of_node;
	int ret = 0;

	ret = of_get_named_gpio(np, "bmm050,irq-gpio", 0);
	if (ret < 0) {
		PERR("failed to get \"bmm050,irq-gpio\"");
		goto err;
	}
	data->irq_gpio = ret;

	ret = of_get_named_gpio(np, "bmm050,drdy-gpio", 0);
	if (ret < 0) {
		PERR("failed to get \"bmm050,drdy-gpio\"");
		goto err;
	}
	data->drdy_gpio = ret;
...
static int bmm050_power_on(struct bmm_client_data *data, bool on)
{
	int rc = 0;
	...
	if (rc) {
		PERR("Regulator vdd enable failed rc=%d", rc);
	}

	rc = regulator_enable(data->vcc_i2c);
	if (rc) {
		PERR("Regulator vcc_i2c enable failed rc=%d", rc);
...
power_off:
	...
	if (rc) {
		PERR("Regulator vdd disable failed rc=%d", rc);
		return rc;
	}

	rc = regulator_disable(data->vcc_i2c);
	if (rc) {
		PERR("Regulator vcc_i2c disable failed rc=%d", rc);
		rc = regulator_enable(data->vdd);
		return rc;
	}
...
static int bmm050_power_init(struct bmm_client_data *data, bool on)
{
	int rc;
	...
	if (IS_ERR(data->vdd)) {
		rc = PTR_ERR(data->vdd);
		PERR("Regulator get failed vdd rc=%d", rc);
		return rc;
	}
	...
	if (regulator_count_voltages(data->vdd) > 0) {
		rc = regulator_set_voltage(data->vdd, FT_VTG_MIN_UV,
					   FT_VTG_MAX_UV);
		if (rc) {
			PERR("Regulator set_vtg failed vdd rc=%d", rc);
			goto reg_vdd_put;
		}
	}
	...
	if (IS_ERR(data->vcc_i2c)) {
		...
		PERR("Regulator get failed vcc_i2c rc=%d", rc);
	...
	if (regulator_count_voltages(data->vcc_i2c) > 0) {
		rc = regulator_set_voltage(data->vcc_i2c, FT_I2C_VTG_MIN_UV,
					   FT_I2C_VTG_MAX_UV);
		if (rc) {
			PERR("Regulator set_vtg failed vcc_i2c rc=%d", rc);
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = bmm050_parse_dt(&client->dev, client_data);
	if (err < 0) {
                PERR("failed to parse device tree: %d", err);
		goto exit_err_clean;
        }
	PINFO("drdy_gpio = %d, irq_gpio = %d", client_data->drdy_gpio, client_data->irq_gpio);

	err = bmm050_power_init(client_data, true);
	if (err) {
		PERR("power init failed");
		goto exit_err_clean;
	}

	err = bmm050_power_on(client_data, true);
	if (err) {
