	...
	int16_t next_lens_position, uint32_t hw_params, uint16_t delay)
{
	...
	CDBG("Enter\n");
	CDBG("actuator ,a_ctrl->pdev->name:%s",a_ctrl->pdev->name);
	for (i = 0; i < size; i++) {
...
ssize_t proc_ois_write (struct file *file, const char __user *buf, size_t nbytes, loff_t *ppos)
{
	...
	if (!strcmp((const char *)string, (const char *)"on")){
		...
		rh63163_ois_enable(1); // Change SCENE parameter OIS-ON
		CDBG("ois on called -\n");
	}else if (!strcmp((const char *)string, (const char *)"off")){
		pr_err("ois off called + \n");
		rh63163_ois_enable(0);  // Change SCENE parameter OIS-Off
		CDBG("ois off called -\n");
	}
	else{
		int dac =atoi(string) ;
		CDBG("proc_ois_write dac %d -\n", dac);
		I2C_OIS_F0123_wr_( 0x90,0x00, dac );					// AF Control ( Value is example )
	}
	return nbytes;
}
	...
	void __user *argp)
{
	...
	int32_t rc = 0;
	/****gaohw4 add for OIS init begin****/
	#ifdef RHM_OIS_ACTUATOR
	extern int16_t rh63163_init( void );
	#endif
	/****gaohw4 add for OIS init end****/
