...
int mmc_get_cd_status(struct mmc_host *host);
static ssize_t card_slot_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct mmc_host *host = cls_dev_to_mmc_host(dev);
	return snprintf(buf, PAGE_SIZE, "%u\n", mmc_get_cd_status(host));
}

DEVICE_ATTR(slot_status, S_IRUGO, card_slot_show, NULL);
static struct attribute *dev_attrs[] = {
#ifdef CONFIG_MMC_PERF_PROFILING
	&dev_attr_perf.attr,
#endif
	&dev_attr_slot_status.attr,
	NULL,
};
