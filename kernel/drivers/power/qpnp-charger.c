...
#if 1 // for bq2753x+bq241xx
    enable = 0;//wyh added always disable qualcomm pmic charger
#endif
...
#if 1 // for bq2753x+bq241xx
    disable = 1;//wyh added always disable get current form usb or ac cahrger
#endif
	return qpnp_chg_masked_write(chip, chip->chgr_base + CHGR_CHG_CTRL,
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	printk("%s:1 is_otg: %d\n", __func__, is_otg);
	if (cclogic_device_is_shenqiaudio()) {
		is_otg = 1;
		return IRQ_HANDLED;
	} else {
		if (is_otg == 1) {
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	pr_info("success chg_dis = %d, bpd = %d, usb = %d, dc = %d b_health = %d batt_present = %d batt_id = %ld\n",
			chip->charging_disabled,
			chip->bpd_detection,
			qpnp_chg_is_usb_chg_plugged_in(chip),
			qpnp_chg_is_dc_chg_plugged_in(chip),
