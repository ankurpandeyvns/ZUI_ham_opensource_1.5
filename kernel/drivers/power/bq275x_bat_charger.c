...
#endif
bool cclogic_device_is_shenqiaudio(void);
...
static void bq27x00_update(struct bq27x00_device_info *di)
{
	...
	if(di->mains_online){
		if(g_floated_charger) {
			printk("%s: floated charger, set low current limit\n", __func__);
...
#if 0
	if(((di->cache.temperature <= (2381 - BQ_BATT_TEMP_OFFSET)) && (force_check_thermal != 1)) || is_testmode)
            val->intval = 255;
        else
...
static irqreturn_t charge_irq(int irq, void *devid)
{
	...
	if (!wake_lock_active(&di->wakelock))
		wake_lock(&di->wakelock);
	/* for otg devices */
	if ((chrg_status & 0xc0) == 0xc0 || cclogic_device_is_shenqiaudio())
		return IRQ_HANDLED;
		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CAPACITY:
			if(((di->cache.temperature <= (2381 - BQ_BATT_TEMP_OFFSET)) && (force_check_thermal != 1)) || is_testmode || di->is_rom_mode || !di->fw_dw_done || di->soc_reset)
				val->intval = 50;
			else{
				ret = bq27x00_read(di, BQ27x00_REG_FLAGS, false);
				batt_present = (ret & BQ27500_FLAG_BAT_DET)>0 ? 1:0;
...
static void bq27x00_boot_work(struct work_struct *work)
{
	...
	printk("%s: testmode=%d, boot_done=%d, force_check_thermal=%d\n", __func__, is_testmode, boot_done, force_check_thermal);
	return;
}
		...
		const struct i2c_device_id *id)
{
	...
	di->chg_state = POWER_SUPPLY_STATUS_UNKNOWN;
...
static int bq27531_config_charging_current(struct bq27x00_device_info *di, int level)
{
	printk("%s: chrg_type=%d, level=%d, float_charger=%d, vbus_ovp=%d, call=%d\n",
		__func__, di->chrg_type, level, g_floated_charger, di->vbus_ovp, g_call_status);

	if ((di->vbus_ovp == 1) || (g_floated_charger == 1))
		return 0;
				...
				unsigned long event, void *data)
{
	...
	//printk("%s: event is %ld\n", __func__, event); // wangwq14 remove this line. too many printf
