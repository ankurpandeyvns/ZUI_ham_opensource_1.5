...
#define ANC_HEADSET_DATA_SHIFT 8
#define ANC_HEADSET_R_DATA 1<<((sizeof(unsigned int)<<3)-1)
#define ANC_HEADSET_R_DATA_SHIFT ((sizeof(unsigned int)<<3)-1)
#define ANC_HEADSET_MIC_DATA 1<<((sizeof(unsigned int)<<3)-1-1)
#define ANC_HEADSET_MIC_DATA_SHIFT ((sizeof(unsigned int)<<3)-1-1)
...
#if 0 /* This gpio used by ANC_MIC */
    {
...
static bool msm8974_headset_swap_america_europe (struct snd_soc_codec *codec,  unsigned char america_europe)
{
	struct snd_soc_card *card = codec->card;
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);

	printk("%s: america_europe swap select switch %d\n", __func__, america_europe);
	gpio_set_value_cansleep(pdata->us_euro_gpio, america_europe);

    gnd_mic_gpio_state = america_europe;
...
#if 0
void msm8974_usb2_sw_en_set(struct snd_soc_card *card,  bool en1,  bool en2)
{
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);

	if(pdata->usb2_sw_en1 && pdata->usb2_sw_en2){
		printk("%s: usb2_sw_en1:%d usb2_sw_en2:%d\n", __func__, en1, en2);
		gpio_set_value_cansleep(pdata->usb2_sw_en1, en1);
		gpio_set_value_cansleep(pdata->usb2_sw_en2, en2);
	}
}
#endif
void msm8974_anc_headset_en_set(struct snd_soc_card *card,  bool audio_r,  bool audio_mic)
{
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);

	pr_debug("%s: audio_r:%d,audio_mic:%d\n", __func__, audio_r, audio_mic);
	//gpio_set_value_cansleep(pdata->audio_r_cmd_sw, audio_r);
	gpio_set_value_cansleep(pdata->audio_mic_cmd_sw, audio_mic);
}

static int msm8974_prepare_anc_headset_en(struct snd_soc_card *card)
{
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	int ret;
	if (pdata->audio_r_cmd_sw >= 0) {
		dev_dbg(card->dev, "%s : AUDIO_R_CMD_SW gpio request %d", __func__,
		pdata->audio_r_cmd_sw);
		ret = gpio_request(pdata->audio_r_cmd_sw, "AUDIO_R_CMD_SW");
		if (ret) {
			dev_err(card->dev,
			"%s: Failed to request AUDIO_R_CMD_SW gpio %d error %d\n",
			__func__, pdata->audio_r_cmd_sw, ret);
			return ret;
		}
		gpio_direction_output(pdata->audio_r_cmd_sw, 0);
	}
	if (pdata->audio_mic_cmd_sw >= 0) {
		dev_dbg(card->dev, "%s : AUDIO_MIC_CMD_SW gpio request %d", __func__,
		pdata->audio_mic_cmd_sw);
		ret = gpio_request(pdata->audio_mic_cmd_sw, "AUDIO_MIC_CMD_SW");
		if (ret) {
			dev_err(card->dev,
			"%s: Failed to request AUDIO_MIC_CMD_SW gpio %d error %d\n",
			__func__, pdata->audio_mic_cmd_sw, ret);
			return ret;
		}
		gpio_direction_output(pdata->audio_mic_cmd_sw, 0);
	}
	if (pdata->anc_audio_cmd >= 0) {
		dev_dbg(card->dev, "%s : ANC_AUDIO_CMD gpio request %d", __func__,
		pdata->anc_audio_cmd);
		ret = gpio_request(pdata->anc_audio_cmd, "ANC_AUDIO_CMD");
		if (ret) {
			dev_err(card->dev,
			"%s: Failed to request ANC_AUDIO_CMD gpio %d error %d\n",
			__func__, pdata->anc_audio_cmd, ret);
			return ret;
		}
		gpio_direction_input(pdata->anc_audio_cmd);
	}
	return 0;
}
#if 0
static int msm8974_prepare_usb2_sw_en(struct snd_soc_card *card)
{
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	int ret;
	if (pdata->usb2_sw_en1 >= 0) {
		dev_dbg(card->dev, "%s : USB2_SW_EN1 gpio request %d", __func__,
		pdata->usb2_sw_en1);
		ret = gpio_request(pdata->usb2_sw_en1, "USB2_SW_EN1");
		if (ret) {
			dev_err(card->dev,
			"%s: Failed to request USB2_SW_EN1 gpio %d error %d\n",
			__func__, pdata->usb2_sw_en1, ret);
			return ret;
		}
		gpio_direction_output(pdata->usb2_sw_en1, 0);
	}
	if (pdata->usb2_sw_en2 >= 0) {
		dev_dbg(card->dev, "%s : USB2_SW_EN2 gpio request %d", __func__,
		pdata->usb2_sw_en2);
		ret = gpio_request(pdata->usb2_sw_en2, "USB2_SW_EN2");
		if (ret) {
			dev_err(card->dev,
			"%s: Failed to request USB2_SW_EN2 gpio %d error %d\n",
			__func__, pdata->usb2_sw_en2, ret);
			return ret;
		}
		gpio_direction_output(pdata->usb2_sw_en1, 0);
	}
...
void msm8974_typec_usb_set( bool en1,  bool en2)
{
#if 0
	msm8974_usb2_sw_en_set(&snd_soc_card_msm8974,  en1,  en2);
#endif
}

  void ow_dir_out(bool b)
 {
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(&snd_soc_card_msm8974);
	gpio_direction_output(pdata->anc_audio_cmd, b);
	gpio_set_value_cansleep(pdata->anc_audio_cmd, b);
 }
  ...
  void ow_dir_in(void)
 {
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(&snd_soc_card_msm8974);
	gpio_direction_input(pdata->anc_audio_cmd);
	gpio_set_value(pdata->anc_audio_cmd, 1);
	usleep(10); 
	pr_debug("%s: anc_audio_cmd:[%d]\n", __func__, gpio_get_value_cansleep(pdata->anc_audio_cmd));
 }

bool ow_get_in(void)
{
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(&snd_soc_card_msm8974);
	pr_debug("%s: Get anc_audio_cmd:[%d]\n", __func__, gpio_get_value_cansleep(pdata->anc_audio_cmd));
	return gpio_get_value_cansleep(pdata->anc_audio_cmd);
}
 
unsigned char ow_bit_io( unsigned char b )
 {
	ow_dir_out(0); // drive bus low

	usleep(10); // Recovery-Time wuffwuff was 1
...
unsigned char ow_byte_wr( unsigned int b )
 {
	uint8_t i = 8, j;
	do{
		j = ow_bit_io( b & 1);
...
uint8_t ow_reset(void)
{
	...
	ow_dir_out(0); // pull OW-Pin low for 480us
...
int msm8974_anc_headset_set(unsigned int arg)
{
	char audio_r = 0;
	char audio_mic = 0;
	unsigned long flags;
	//unsigned char anc_data = 0;
	unsigned char anc_ack = 0;
	int i;
	struct msm8974_asoc_mach_data *pdata = snd_soc_card_get_drvdata(&snd_soc_card_msm8974);
	...
	if (audio_r && audio_mic){
		pr_err("Those line do not set all High\n");
		return -EINVAL;
	}
	
	printk("[%s]Write data:...\n", __func__);
	spin_lock_irqsave(&anc_lock,flags);
	msm8974_anc_headset_en_set(&snd_soc_card_msm8974, audio_r, audio_mic);
	udelay(500);
	for(i=0;i<(sizeof(unsigned char)<<3)+4;i++){
		//printk("[%s][%d]\n", __func__,(((0xFFFFF&arg>>ANC_HEADSET_DATA_SHIFT)<<i)&0x80000)>>(20-1));
		//msm8974_anc_headset_en_set(&snd_soc_card_msm8974, audio_r, audio_mic, (((0xFFFF&arg>>ANC_HEADSET_DATA_SHIFT)<<i)&0x8000)>>16-1);
		gpio_set_value(pdata->anc_audio_cmd,(((0xFFFFF&arg>>ANC_HEADSET_DATA_SHIFT)<<i)&0x80000)>>(20-1));/*  MSB Output */
		udelay(500);
	}
	gpio_set_value(pdata->anc_audio_cmd,0);

	//printk("[%s]Write data done\n", __func__);

	gpio_set_value(pdata->anc_audio_cmd,1);
	gpio_set_value(pdata->anc_audio_cmd,0);

	/* To read ACK for write */
	gpio_direction_input(pdata->anc_audio_cmd);
	udelay(300);
	i = 7;
	do{
		if(gpio_get_value(pdata->anc_audio_cmd)){
			anc_ack |=1;
		}
	...
	udelay(200);
	gpio_direction_output(pdata->anc_audio_cmd, 0);
	gpio_set_value(pdata->anc_audio_cmd,1);
	gpio_set_value(pdata->anc_audio_cmd,0);
	msm8974_anc_headset_en_set(&snd_soc_card_msm8974, 0, 0);
	spin_unlock_irqrestore(&anc_lock,flags);
	printk("[%s]anc_ack=%#x\n", __func__,  anc_ack);
...
static __devinit int msm8974_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (pdata->us_euro_gpio < 0) {
		...
		mbhc_cfg.headset_swap_america_europe = msm8974_headset_swap_america_europe;
	}
	...
	if (ret)
		dev_err(&pdev->dev, "msm8974_prepare_us_euro failed (%d)\n",
			ret);
	pdata->usb2_sw_en1 = of_get_named_gpio(pdev->dev.of_node,
	                       "qcom,usb2_sw_en1", 0);
	if (pdata->usb2_sw_en1 < 0) {
	       dev_info(&pdev->dev, "property %s not detected in node %s",
	               "qcom,usb2_sw_en1",
	       ...
	       dev_dbg(&pdev->dev, "%s detected %d",
	               "qcom,usb2_sw_en1", pdata->usb2_sw_en1);
	}

	pdata->usb2_sw_en2 = of_get_named_gpio(pdev->dev.of_node,
	                       "qcom,usb2_sw_en2", 0);
	if (pdata->usb2_sw_en2 < 0) {
	       dev_info(&pdev->dev, "property %s not detected in node %s",
	               "qcom,usb2_sw_en2",
	       ...
	       dev_dbg(&pdev->dev, "%s detected %d",
	               "qcom,usb2_sw_en2", pdata->usb2_sw_en2);
	}
#if 0
	ret = msm8974_prepare_usb2_sw_en(card);
	if (ret)
	       dev_err(&pdev->dev, "msm8974_prepare_us_euro failed (%d)\n",
	               ret);

	msm8974_usb2_sw_en_set(card, 1, 1); /* HIGH HIGH for USB, HIGH LOW for Audio */
#endif
	pdata->audio_r_cmd_sw = of_get_named_gpio(pdev->dev.of_node,
	                       "qcom,audio_r_cmd_sw", 0);
	if (pdata->audio_r_cmd_sw < 0) {
	       dev_info(&pdev->dev, "property %s not detected in node %s",
	               "qcom,audio_r_cmd_sw",
	               pdev->dev.of_node->full_name);
	       ...
	       dev_dbg(&pdev->dev, "%s detected %d",
	               "qcom,audio_r_cmd_sw", pdata->audio_r_cmd_sw);
	}

	pdata->audio_mic_cmd_sw = of_get_named_gpio(pdev->dev.of_node,
	                       "qcom,audio_mic_cmd_sw", 0);
	if (pdata->audio_mic_cmd_sw < 0) {
	       dev_info(&pdev->dev, "property %s not detected in node %s",
	               "qcom,audio_mic_cmd_sw",
	               pdev->dev.of_node->full_name);
	} else {
	       dev_dbg(&pdev->dev, "%s detected %d",
	               "qcom,audio_mic_cmd_sw", pdata->audio_mic_cmd_sw);
	}

	pdata->anc_audio_cmd = of_get_named_gpio(pdev->dev.of_node,
	                       "qcom,anc_audio_cmd", 0);
	if (pdata->anc_audio_cmd < 0) {
	       dev_info(&pdev->dev, "property %s not detected in node %s",
	               "qcom,anc_audio_cmd",
	               pdev->dev.of_node->full_name);
	       ...
	       dev_dbg(&pdev->dev, "%s detected %d",
	               "qcom,anc_audio_cmd", pdata->anc_audio_cmd);
	}
	ret = msm8974_prepare_anc_headset_en(card);
	if (ret)
	       dev_err(&pdev->dev, "msm8974_prepare_anc_headset_en failed (%d)\n",
	               ret);

	msm8974_anc_headset_en_set(card, 0, 0); /* LOW for NCx Connected to COMx, HIGH for NOx Connected to COMx */
