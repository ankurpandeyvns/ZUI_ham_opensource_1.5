...
#define CONFIG_HOSTAPD_CTRL_IFACE "/data/misc/wifi/hostapd"
...
int main(int argc, char *argv[])
{
	...
	if (os_program_init())
		return -1;
	//make dir for softap
	if((dir = opendir(CONFIG_HOSTAPD_CTRL_IFACE)) == NULL) {
		mkdir(CONFIG_HOSTAPD_CTRL_IFACE,S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IXUSR);
		chown(CONFIG_HOSTAPD_CTRL_IFACE, 1010, 1010);
		chmod(CONFIG_HOSTAPD_CTRL_IFACE, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP| S_IXGRP | S_IXUSR);
	} else {
		closedir(dir);
	}

	if((dir = opendir(CONFIG_CTRL_IFACE_CLIENT_DIR)) == NULL) {
		mkdir(CONFIG_CTRL_IFACE_CLIENT_DIR,S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IXUSR);
		chown(CONFIG_CTRL_IFACE_CLIENT_DIR, 1010, 1010);
		chmod(CONFIG_CTRL_IFACE_CLIENT_DIR, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP| S_IXGRP | S_IXUSR);
	} else {
		closedir(dir);
	}
