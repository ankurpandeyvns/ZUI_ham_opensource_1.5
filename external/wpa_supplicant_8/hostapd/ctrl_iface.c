...
/*Begin, SHENQI zhenglq1, WiFi Hotspot Manager Support*/
	...
	os_snprintf(buf, len, "%s/%s",
		    hapd->conf->ctrl_interface, "ap0");
#else
	os_snprintf(buf, len, "%s/%s",
		    hapd->conf->ctrl_interface, hapd->conf->iface);
#endif
        /*End, SHENQI zhenglq1, WiFi Hotspot Manager Support*/
...
int hostapd_ctrl_iface_init(struct hostapd_data *hapd)
{
	...
	if (os_strlen(hapd->conf->ctrl_interface) + 1 +
	    os_strlen("ap0") >= sizeof(addr.sun_path))
		goto fail;
#else
	if (os_strlen(hapd->conf->ctrl_interface) + 1 +
