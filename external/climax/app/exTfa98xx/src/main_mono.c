...
#define SPEAKER_FILENAME "shenqi.speaker"
#define PRESET_FILENAME "shenqi_music.preset"
#define CONFIG_FILENAME "TFA9890_N1B12_N1C3_v2.config"
#define EQ_FILENAME "shenqi_music.eq"
...
int typec_usb_set(unsigned int enable)
{
	int fd, ret_value=0,ret=0;
	...
	ret_value = ioctl(fd, ANC_HEADSET_SET, enable);
	if ( ret < 0 ) {
...
int audio_smartpa_resetMtpEx(void)
{
	int fd, ret=0;
	Tfa98xx_handle_t handle;
	...
	assert(ret == Tfa98xx_Error_Ok);
	TFA_LOGD("SmartPA resetMtpEx");
	g_CalibrationStatus = 0;
	resetMtpEx(handle);
	muteAmplifier(handle);
	Tfa98xx_Powerdown(handle, 1);
...
int main(int argc, char* argv[])
{
    ...
    if(argc >= 1){
        if(strcmp("SmartPA ON", argv[1])==0){
			...
			read_tfa98xx_id(&id);
		}else if(strcmp("SmartPA INIT", argv[1])==0){
			tfa9890_init(48000);
		}else if(strcmp("SmartPA RESETMTPEX", argv[1])==0){
			audio_smartpa_resetMtpEx();
		}
    }
...
int EQset_Impl(int mode)
{
	...
	if (handle != -1) {
	    switch(mode){
	        case 0:{//normal
	            setPreset(handle,"/etc/tfa98xx/shenqi_music.preset");
	            setEQ(handle,"/etc/tfa98xx/shenqi_music.eq");
	            ALOGI("[%s] set normal preset and eq.", __func__);
	            break;
	        }
			case 1:{//incall
			    setPreset(handle,"/etc/tfa98xx/shenqi_speech.preset");
	            setEQ(handle,"/etc/tfa98xx/shenqi_speech.eq");
...
int init(int sRate)
{
    ...
    TFA_LOGD("Calibration TFA98XX_NOMINAL_IMPEDANCE:%2.2f ohm\n",TFA98XX_NOMINAL_IMPEDANCE);
    TFA_LOGD("Calibration TFA98XX_NOMINAL_IMPEDANCE_MIN:%2.2f ohm TFA98XX_NOMINAL_IMPEDANCE_MAX:%2.2f\n", TFA98XX_NOMINAL_IMPEDANCE_MIN, TFA98XX_NOMINAL_IMPEDANCE_MAX);
...
void tfa9890_SpeakerOn(void)
{
    ...
    if ((-1 == handle) || (0 == g_CalibrationStatus)) {
	    rc = init(g_sRate);
		if (0 != rc) {
			ALOGE("%s:%u init error = %d",__func__, __LINE__,rc);
			return;
